    ...
    <div className="p-8 space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              <h1 className="text-xl font-bold text-white">{scan.target_name}</h1>
              <span className="text-[11px] font-bold px-2 py-0.5 rounded bg-cyan-950 text-cyan-400 border border-cyan-500/30 uppercase">
                {scan.scan_type}
              </span>
              <span className={`text-[11px] font-bold px-2 py-0.5 rounded border uppercase ${
                ...
                {scan.scan_engine === 'zap' ? '🛡️ OWASP ZAP Engine' : '⚡ Fallback Engine'}
              </span>
            </div>
            <a
              ...
              className="text-xs font-mono text-slate-400 hover:text-cyan-400 flex items-center gap-1 mt-0.5"
            >
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 rounded-lg bg-slate-850 hover:bg-slate-800 border border-cyan-500/40 text-cyan-300 text-xs font-bold uppercase tracking-wider transition-all disabled:opacity-50 shadow-glow-cyan"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="cyber-card p-6 flex items-center justify-around">
          <ScoreGauge score={scan.security_score || 100} size={150} label="Scan Score" />
          <div className="space-y-2 text-xs">
            <div className="flex items-center gap-1.5 text-slate-400">
              <CheckCircle2 className="w-4 h-4 text-emerald-400" />
              <span>Status: <strong className="text-white uppercase">{scan.status}</strong></span>
            </div>
            <div className="flex items-center gap-1.5 text-slate-400">
        ...
        {/* Severity Metrics */}
        <div className="cyber-card p-6 col-span-2 flex flex-col justify-between">
          <div>
            ...
            <div className="grid grid-cols-5 gap-3 mt-4 text-center">
              <div className="p-3 rounded-lg bg-red-950/40 border border-red-500/30">
                <span className="text-[10px] uppercase font-bold text-red-400 block">Critical</span>
                <span className="text-xl font-black text-white">{scan.severity_breakdown?.critical || 0}</span>
              </div>
              <div className="p-3 rounded-lg bg-orange-950/40 border border-orange-500/30">
                <span className="text-[10px] uppercase font-bold text-orange-400 block">High</span>
                <span className="text-xl font-black text-white">{scan.severity_breakdown?.high || 0}</span>
              </div>
              <div className="p-3 rounded-lg bg-amber-950/40 border border-amber-500/30">
                ...
                <span className="text-xl font-black text-white">{scan.severity_breakdown?.medium || 0}</span>
              </div>
              <div className="p-3 rounded-lg bg-blue-950/40 border border-blue-500/30">
                <span className="text-[10px] uppercase font-bold text-blue-400 block">Low</span>
                <span className="text-xl font-black text-white">{scan.severity_breakdown?.low || 0}</span>
              </div>
              <div className="p-3 rounded-lg bg-slate-900 border border-slate-700">
                ...
                <span className="text-xl font-black text-white">{scan.severity_breakdown?.informational || 0}</span>
              </div>
            </div>
          </div>

          <div className="text-[11px] text-slate-500 pt-3 border-t border-slate-800 flex justify-between">
            <span>Deterministic Scoring Formula Applied</span>
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
        <div className="relative w-full sm:w-80">
          ...
          <input
            type="text"
            placeholder="Search finding name, OWASP category, URL..."
            value={searchQuery}
            ...
            {filteredFindings.length === 0 ? (
              <tr>
                <td colSpan={5} className="py-8 text-center text-slate-500">
                  No vulnerability findings match your search or filter.
                </td>
              </tr>
            ) : (
